...
function processRTFContent(content) {
  ...
  const endingExtractionResult = extractEndingTags(modifiedContent);

  modifiedContent = endingExtractionResult.modifiedContent;
  let endArray = endingExtractionResult.endArray;

  const italicExtractionResult = extractItalicSections(modifiedContent);
...
function reorganizeItalicSection(content) {
  ...
  const extractedTitle = extractTitleOfPlay(modifiedContent); //finds Title of Play and extracts it into its own array
...
function extractCharacterTags(content) {
  content = condenseCharacterNames(content);
...
function condenseCharacterNames(content) {
  const lines = content.split("\n");
  const condensedLines = [];

  const nameRegex = /^([A-Z\s]+)(\{[^}]*\})?\s*$/;

  for (let i = 0; i < lines.length; i++) {
    const match = lines[i].match(nameRegex);
    if (match) {
      const characterLine = match[1].trim() + ".";
      const additionalText = match[2] ? ` ${match[2]}` : "";
      const nextLine = lines[i + 1] ? ` ${lines[i + 1]}` : "";
      condensedLines.push(characterLine + additionalText + nextLine);
      i++; // skip the next line since it's already included
    } else {
      condensedLines.push(lines[i]);
    }
  }
...
function createOneActStructure(modifiedContent, extractedSections) {
  modifiedContent = modifiedContent
    .replace(/({play1}{pdes1})/, "$1{act1}{ades1}")
...
function matchSceneTags(
  ...
  arrayOfActs.forEach((act) => {
    let actMatches = act.match(/{act(\d+)}/);
    let actNumber = actMatches ? actMatches[1] : null;
    act = act
      .replace(new RegExp(`{act${actNumber}}{ades${actNumber}}`), "")
      .replace(new RegExp(`{enda${actNumber}}`), "");
    ...
    act.forEach((scene, index) => {
      ...
      let sceneMatch = scene.match(sceneRegex);
      ...
      if (!sceneMatch) {
        scene = `{s1}` + scene;
        sceneMatch = scene.match(sceneRegex);
      }
      ...
      if (sceneMatch) {
        ...
        //prettier-ignore
        let extractedScene = extractedSections.match(new RegExp(`${sceneTag} - \\[(.*)\\]`));
        if (extractedScene) {
          sceneTitle = extractedScene[1];
        } else {
          sceneTitle = "Scene 1";
        }
        console.log(sceneTitle);
        //prettier-ignore
        structuredPlayContent.actStructure[0].sceneStructure[sceneCount].sceneTitle = sceneTitle;
        console.log(
          structuredPlayContent.actStructure[0].sceneStructure[sceneCount]
            .sceneTitle
        );
      ...
      //TODO: Add internal trigger for Character and Stage Directions functions
    });
  });
  return { modifiedContent, extractedSections, structuredPlayContent };
}
