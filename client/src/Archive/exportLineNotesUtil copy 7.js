...
const exportLineNotesUtil = (lineNotes, character) => {
  ...
  const createRichLineNotes = (doc, textArray, cellWidth) => {
    ...
    textArray.forEach((part) => {
      const textWidth = cellWidth - doc.getLineHeight() * 2;

      if (part.droppedWords) {
        const textColor = [0, 128, 0];
        ...
        const textColor = [255, 0, 0];
        const fontStyle = "normal";
        const isLineThrough = true;
        handleTextFormatting(
          doc,
          part.text,
          textWidth,
    ...
    function handleTextFormatting(
      ...
      while (doc.getTextWidth(text) > textWidth) {
      ...
      if (x + doc.getTextWidth(text) > cellWidth - doc.getLineHeight() * 2) {
        x = doc.getLineHeight();
        y += doc.getLineHeight();
        lines += 1;
      }
